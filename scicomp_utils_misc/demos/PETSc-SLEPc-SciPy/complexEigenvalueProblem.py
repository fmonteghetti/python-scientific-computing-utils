...
def filterSpectrum_complexResidual(
    A, eigval, eigvec, idx_r, idx_i, tol=1e-16, B=None
):
    x = eigvec[idx_r, :]
    y = eigvec[idx_i, :]
    ...
    for i in range(len(eigval)):
        print(
            f"{np.real(eigval[i]):1.3e}\t{np.imag(eigval[i]):1.3e}\t{res[i]:1.3e}"
        )
...
def filterSpectrum_realResidual(
    A, eigval, eigvec, idx_r, idx_i, tol=1e-16, B=None
):
...
A[0, 0] = 5
A[1:N, 1:N] = np.ones((N - 1, N - 1), dtype=complex) + 1j * np.diag(
    np.ones(N - 1)
)
...
(eigval_ex, eigvec_ex) = filterSpectrum_complexResidual(
    A, eigval_real, eigvec_real, idx_r, idx_i, tol=1e-10, B=B
)
# Filter using real arithmetic
print("--- Filtered eigenvalues (residual / real arithmetic)")
(eigval_ex, eigvec_ex) = filterSpectrum_realResidual(
    A_real, eigval_real, eigvec_real, idx_r, idx_i, tol=1e-10, B=B_real
)
# Filter using real arithmetic and no matrix-vector product
print("--- Filtered eigenvalues (eigvec criterion / real arithmetic)")
(eigval_ex, eigvec_ex) = filterSpectrum_eigenvectors(
    eigval_real, eigvec_real, idx_r, idx_i, tol=1e-14
)
...
# Filter
(eigval, eigvec) = SLEPc_utils.EPS_get_spectrum_ReImFormulation(
    EPS, idx_r.astype(np.int32), idx_i.astype(np.int32)
)
# Print
