...
(A,b) = PoissonFDM.assemble_PETSc(L,N,f,g)
E = PoissonFDM.eigensolve_SLEPc(A,SLEPc.EPS.ProblemType.NHEP,comm=PETSc.COMM_WORLD,nev=10)
...
for j in range(np.min([eigvec.shape[1],5])):
        tmp = np.real(eigvec[:,j]); tmp=tmp/np.linalg.norm(tmp)
        ax.plot(x_FDM[1:-1],tmp,label=f"vec {j}")
ax.set_xlabel("x")
ax.set_ylabel("y")
ax.legend()
    # exact eigenvectors for comparison
tmp = -np.sin(x_FDM*4*pi/L); tmp=tmp/np.linalg.norm(tmp)
ax.plot(x_FDM,tmp,'--')