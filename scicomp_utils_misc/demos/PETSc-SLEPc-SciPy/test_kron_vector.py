...
def _compute_error(A, A_ex):
    ...
    return comm.allreduce(error, op=MPI.SUM)


N = 239
test_cases = [_get_integer_test(N + 27), _get_real_test(N + 28)]
...
for L, A_ex in test_cases:
    ...
    if comm.rank == 0:
        print(
            f"Elapsed time (re-use): {1e-9 * (time.process_time_ns() - t0):1.2g}s"
        )
    error = _compute_error(A, A_ex)
    if comm.rank == 0:
        print(f"Error: {error}")
...
if comm.rank == 0:
    print(
        f"nz_allocated/nz_unneeded/mallocs: {info['nz_allocated']}/{info['nz_unneeded']}/{info['mallocs']}."
    )
