...
class Laplace_spectrum:
    ...
    def rectangle(Lx,Ly,N,bc):
        ...
        eigval = []
        if bc == 'dirichlet':
            eigval = [(nx*np.pi/Lx)**2 + (ny*np.pi/Ly)**2 \
                      for nx in range(1,N+1) for ny in range(1,N+1)]
        elif bc == 'neumann':
            eigval = [(nx*np.pi/Lx)**2 + (ny*np.pi/Ly)**2 \
                      for nx in range(0,N) for ny in range(0,N) \
                          if (nx+ny)>=1]
    ...
    def cube(Lx,Ly,Lz,N,bc):
        ...
        eigval = []
        if bc == "dirichlet":
            eigval = [(nx*np.pi/Lx)**2 + (ny*np.pi/Ly)**2 + (nz*np.pi/Lz)**2 \
                    for nx in range(1,N+1) for ny in range(1,N+1) for nz in range(1,N+1)]
        elif bc == "neumann":
            eigval = [(nx*np.pi/Lx)**2 + (ny*np.pi/Ly)**2 + (nz*np.pi/Lz)**2 \
                    for nx in range(N) for ny in range(N) for nz in range(N) \
                        if nx+ny+nz>=1]
            eigval = np.append(eigval,np.array([0]))
        return np.sort(eigval)[0:N]

    def disc(R,bc,eigval_min=0,eigval_max=10,eigval_N=10,eigval_maxorder=2):
        """
    ...
    def ball(R,bc,eigval_min=0,eigval_max=10,eigval_N=10,eigval_maxorder=2):
        """
...
class Maxwell_spectrum:
    ...
    def ball(R,bc,eigval_min=0,eigval_max=10,eigval_N=10,eigval_maxorder=4):
        ...
        if bc=="pec":
            ...
            g = lambda n, x: scipy.special.spherical_jn(n,x)\
                    - x*scipy.special.spherical_jn(n,x,derivative=True)
            for n in range(eigval_maxorder+1): # Bessel function order
                xmin,xmax = R*eigval_min, R*eigval_max
                dx = (xmax-xmin)/eigval_N
                    # Transverse electric modes
                eigval_TE = scipy_utils.find_zeros(lambda x:f(n,x),xmin,xmax,dx)
                    # Transverse magnetic modes
                eigval_TM = scipy_utils.find_zeros(lambda x:g(n,x),xmin,xmax,dx)
    ...
    def disc_TE(R,bc,eigval_min=0,eigval_max=10,eigval_N=10,eigval_maxorder=2):
        """
        ...
        if bc == 'pec':
                # Non-zero Neumann Laplacian eigenvalues
            eigval = Laplace_spectrum.disc(R,"neumann",eigval_min=eigval_min,
                                           eigval_max=eigval_max,
                                           eigval_N=eigval_N,
                                           eigval_maxorder=eigval_maxorder)[1:]
            eigval = 1j*np.sort(np.sqrt(eigval))
        return eigval

    def disc_TM(R,bc,eigval_min=0,eigval_max=10,eigval_N=10,eigval_maxorder=2):
        """
        ...
        if bc == 'pec':
            ...
            eigval = Laplace_spectrum.disc(R,"dirichlet",eigval_min=eigval_min,
                                           eigval_max=eigval_max,
                                           eigval_N=eigval_N,
