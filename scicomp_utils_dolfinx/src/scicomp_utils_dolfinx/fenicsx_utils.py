...
def create_DirichletBC(
    mesh, facet_tags, V, uDs, uD_facet_tags, idx_sub_space=[]
):
    """
    ...
    if len(uDs) != len(uD_facet_tags):
        raise ValueError(
            "Lists defining Dirichlet boundary conditions have different sizes."
        )
    bcs = list()
    for i in range(len(uD_facet_tags)):
        ...
        if len(idx_sub_space) == 0:  # uDs[i] belongs to V
            bnd_dof = dolfinx.fem.locate_dofs_topological(
                V, mesh.topology.dim - 1, bnd_facets
            )
            bcs.append(dolfinx.fem.dirichletbc(uDs[i], bnd_dof))
        else:  # uDs[i] belongs to V.sub(idx_sub_space[i])
            idx = idx_sub_space[i]
            bnd_dof = dolfinx.fem.locate_dofs_topological(
                (V.sub(idx), V.sub(idx).collapse()[0]),
                mesh.topology.dim - 1,
                bnd_facets,
            )
...
def assemble_GEP(a, b, bcs, diag_A=1e2, diag_B=1e-2):
    ...
    """
    A = dolfinx.fem.petsc.assemble_matrix(dolfinx.fem.form(a), bcs, diag_A)
    A.assemble()
    B = dolfinx.fem.petsc.assemble_matrix(dolfinx.fem.form(b), bcs, diag_B)
    B.assemble()
    return (A, B)
...
def assemble_GEP_block(a, b, bcs, diag_A=1e2, diag_B=1e-2):
    ...
    A = dolfinx.fem.petsc.assemble_matrix_block(
        dolfinx.fem.form(a), bcs, diag_A
    )
    A.assemble()
    B = dolfinx.fem.petsc.assemble_matrix_block(
        dolfinx.fem.form(b), bcs, diag_B
    )
    B.assemble()
    return (A, B)
