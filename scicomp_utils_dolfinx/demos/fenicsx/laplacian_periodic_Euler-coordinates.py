...
def map_RectLeftBnd_to_DiskBnd(x):
    y = np.zeros(x.shape)
    y[0] = R_TR * np.cos(x[1] - y_offset)
    y[1] = R_TR * np.sin(x[1] - y_offset)
    y[2] = x[2]
    return y
...
bcs = fenicsx_utils.create_DirichletBC(
    dmesh.mesh, dmesh.facet_tags, V, [uD], Dirichlet_bnd
)
# Periodic boundary condition
...
m = ufl.inner(u, v) * dx(phys_tag_2D["Disk"][0]) + weight * ufl.inner(
    u, v
) * dx(phys_tag_2D["Rectangle"][0])
f = PETSc.ScalarType(-6)
l = ufl.inner(f, v) * dx(phys_tag_2D["Disk"][0]) + weight * ufl.inner(
    f, v
) * dx(phys_tag_2D["Rectangle"][0])
