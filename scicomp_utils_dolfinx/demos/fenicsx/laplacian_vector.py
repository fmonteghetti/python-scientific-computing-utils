...
gmsh_utils.generate_mesh_cli(
    ...
    refinement=0,
    binary=True,
    parameters={"x_l": x_left, "x_r": x_right, "N_x": 50, "N_y": 50},
    order=mesh_order,
)
...
V = dolfinx.fem.functionspace(
    dmesh.mesh, ("CG", mesh_order, (dmesh.mesh.geometry.dim,))
)
...
a = (
    ufl.inner(ufl.grad(u[0]), ufl.grad(phi[0])) * ufl.dx
    + ufl.inner(ufl.grad(u[1]), ufl.grad(phi[1])) * ufl.dx
)
