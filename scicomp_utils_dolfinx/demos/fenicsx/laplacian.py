...
if comm.rank == 0:
    print(
        f"Elapsed time (direct problem): {1e-9 * (time.process_time_ns() - t0):1.2g}s"
    )
...
with dolfinx.io.XDMFFile(
    mesh.comm, "shared/demo_laplace_direct.xdmf", "w"
) as file:
...
BDM = basix.ufl.element(
    "BDM", dmesh.mesh.ufl_cell().cellname(), degree=degree_fem
)
DG = basix.ufl.element(
    "DG", dmesh.mesh.ufl_cell().cellname(), degree=degree_fem - 1
)
...
problem = dolfinx.fem.petsc.LinearProblem(
    a, l, petsc_options={"ksp_type": "gmres", "pc_type": "bjacobi"}
)
Uh = problem.solve()
comm.Barrier()
if comm.rank == 0:
    print(
        f"Elapsed time (direct problem, mixed): {1e-9 * (time.process_time_ns() - t0):1.2g}s"
    )
...
V_proj = dolfinx.fem.functionspace(
    mesh, ("DG", degree_fem - 1, (dmesh.mesh.geometry.dim,))
)
phih_proj = fenicsx_utils.project(
    phih, V_proj, petsc_options={"ksp_type": "preonly", "pc_type": "lu"}
)
...
if comm.rank == 0:
    print(
        f"Elapsed time (eigenvalue problem): {1e-9 * (time.process_time_ns() - t0):1.2g}s"
    )
...
for d in range(dim + 1):
    mpi_print(
        f"# of local (global) element of dim={d}: {mesh.topology.index_map(d).size_local} ({mesh.topology.index_map(d).size_global})"
    )
    # Each process stores 'local' and 'ghost' dofs of functions in V.
mpi_print(
    f"# of ghost| local | global dofs: {V.dofmap.index_map.num_ghosts} | {V.dofmap.index_map.size_local} | {V.dofmap.index_map.size_global}"
)
...
mpi_print(
    f"Local size of uh.x.petsc_vec (= # of local dofs): {uh.x.petsc_vec.sizes[0]}"
)
