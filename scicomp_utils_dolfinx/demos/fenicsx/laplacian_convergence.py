...
plotter.add_text(
    f"{gmshfile}\nNodes: {dmesh.mesh.geometry.x.shape[0]}",
    position="upper_edge",
    font_size=8,
    color="black",
)
...
bcs = fenicsx_utils.create_DirichletBC(
    dmesh.mesh, dmesh.facet_tags, V, [uD], [phys_tag_1D["Gamma-ext"]]
)
# Weak formulation
...
a = ufl.inner(ufl.grad(u), ufl.grad(v)) * dmesh.dx(
    phys_tag_2D["Omega-int"][0]
) + ufl.inner(kappa * ufl.grad(u), ufl.grad(v)) * dmesh.dx(
    phys_tag_2D["Omega-ext"][0]
)
b = ufl.inner(u, v) * dmesh.dx
L = ufl.inner(f, v) * dmesh.dx(phys_tag_2D["Omega-int"][0]) + ufl.inner(
    f, v
) * dmesh.dx(phys_tag_2D["Omega-ext"][0])
# %% Solve
