...
eigval_dirichlet = np.hstack(
    [scipy.special.jn_zeros(n, 5) ** 2 for n in range(0, 4)]
)
# Eigenvalues of Neumann Laplacian on disc of radius 1
eigval_neumann = np.hstack(
    [scipy.special.jnp_zeros(n, 5) ** 2 for n in range(0, 4)]
)
...
K = dolfinx.fem.petsc.assemble_matrix(dolfinx.fem.form(k))
K.assemble()
M = dolfinx.fem.petsc.assemble_matrix(dolfinx.fem.form(m))
M.assemble()
C = dolfinx.fem.petsc.assemble_matrix(dolfinx.fem.form(c))
C.assemble()
PEP = SLEPc_utils.solve_PEP_shiftinvert(
    [K, C, M], comm=comm, nev=20, tol=1e-6, max_it=100, target=4, shift=4
)
...
ax.plot(
    np.real(np.sqrt(eigval_dirichlet)),
    np.imag(np.sqrt(eigval_dirichlet)),
    label="Exact Dirichlet",
    marker="o",
    linestyle="none",
    fillstyle="none",
)
ax.plot(
    np.real(np.sqrt(eigval_neumann)),
    np.imag(np.sqrt(eigval_neumann)),
    label="Exact Neumann",
    marker="s",
    linestyle="none",
    fillstyle="none",
