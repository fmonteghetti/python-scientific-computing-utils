...
if comm.rank == 0:  # generate mesh on first thread only
    gmsh_utils.generate_mesh_cli(
        ...
        refinement=0,
        binary=True,
        parameters={"x_r": L[0], "y_t": L[1], "N_x": 100, "N_y": 100},
    )
comm.Barrier()
...
if bc_vert == "dirichlet":
    ...
    Dirichlet_bnd = [
        t for name in Dirichlet_bnd_name for t in dmesh.name_2_tags[-2][name]
    ]
...
if comm.rank == 0:
    ...
    ax.plot(
        ...
        marker="o",
        linestyle="none",
        label=f"FEM N={V.dofmap.index_map.size_global}",
    )
    ax.plot(
        np.real(ev_ex),
        np.imag(ev_ex),
