...
def create_submesh_from_gmsh(mesh, cell_type, outfile, prune_z=False):
    ...
    if prune_z:
        prune_z_0(out_mesh)
    #        out_mesh.prune_z_0() # deprecated in meshio 7.0.0
...
def read_phys_gmsh(jsonfile):
    ...
    for name in d:
        if len(d[name]) > 2:
            raise ValueError(
                f"JSON file contains more than two integers for '{name}'"
            )
        dim = d[name][1]
...
def print_diagnostic_gmsh(meshfile):
    ...
    for i in range(len(msh.cells)):
        print(
            f"\t#{i} type: '{msh.cells[i].type}' total: {msh.cells[i].data.shape[0]}"
        )
...
def convert_gmsh_to_XDMF(meshfile, prune_z=False):
    ...
    print(
        f"Mesh '{meshfile}' contains cell types:\n\t{list(cell_types.keys())}"
    )
    xdmfiles = dict()
...
def get_phys_gmsh_tag(meshtag, dim, name):
    """
    Get gmsh physical entities tags from from mesthag.

    Parameters
    ----------
    meshtag : dict of str
        Dictionary of gmsh physical entities.
    dim : int
        Physical entities dimension.
    name : list of str
        Physical entities names.

    Returns
    -------
    tags : list of int
        Tags of physical entities.

    """
    # Get all entries
    tagl = [meshtag[dim].get(key) for key in name]
    # Flatten list
    tags = [item for sublist in tagl for item in sublist]
    return tags
