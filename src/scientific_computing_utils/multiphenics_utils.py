...
def get_global_dof(W,idx):
    """
    Get global DoF indices associated with unknown u_idx.
    ...
    """
    dofmap = W.block_dofmap()
    a = [np.array(dofmap.block_owned_dofs__global_numbering(i),dtype=int) for i in idx]
    return np.hstack(a)
...
def export_pvd(pvdfile,sol_idx,sol_name,t,y,W):
    ...
    pvdf = fe.File(pvdfile)
    print(f"Export to {pvdf}...")
    for i in range(len(y)):
        print(f"Step t={t[i]}")
        f = get_function(y[i], 0,"e_u",W)
        fun = fe.Function(W.sub(sol_idx),f.vector(),name=sol_name)
        pvdf << (fun, t[i])
    print("Done.")
