...
def assemble_numpy(L,N,f,g):
    ...
    d = -2*np.ones(N-2); e = 1*np.ones(N-3)
    A = np.diag(d) + np.diag(e, k=1) + np.diag(e, k=-1)
    A = -((1/h)**2)*A
        # source term
    F = f(x_FDM[1:-1])
        # boundary term
    G = np.zeros((N-2,)); G[0]=g(x_FDM[0]); G[-1]=g(x_FDM[-1]);G=((1/h)**2)*G
    return (A,F + G)
...
def assemble_scipy(L,N,f,g):
    ...
    A = sp.spdiags(d,0,N-2,N-2,format="csr")+ A_lower + A_lower.transpose()
    A = -((1/h)**2)*A
        # source term
    F = f(x_FDM[1:-1])
        # boundary term
...
def solve_scipy(A,b,solver="umfpack"):
    ...
    if solver=="umfpack":
        ...
        x = splinalg.gmres(A,b); x = x[0] # GMRES
    else:
        x = splinalg.cg(A,b); x = x[0] # CG
...
def eigensolve_SLEPc(A,ProblemType=SLEPc.EPS.ProblemType.NHEP,comm=None,nev=1):
    """
